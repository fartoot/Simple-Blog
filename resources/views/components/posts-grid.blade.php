        ...
        <div role="list"
            ...
            @foreach ($posts->slice(2) as $key => $post)
                <a href="/post/{{ $post->slug }}">
                    ...
                    <h3 class="mt-6 text-lg font-semibold leading-8 tracking-tight text-gray-900">{{ $post->title }}
                    </h3>
                    <p class="text-base leading-7 text-gray-600">{{ $post->excerpt }}</p>
                    <div class="flex items-center justify-between px-5 mt-6 text-gray-500 ">
                        <div class="flex items-center gap-3">
                            <img class="rounded-full h-7 w-7"
                                src="https://gravatar.com/avatar/205e460b479e2e5b48aec07710c08d50" />
                            <p>Mario Sanchez</p>
